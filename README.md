> Tracking my journey through Udacity's AI master's program

[![Institution](https://img.shields.io/badge/Udacity-Master_of_Science_in_AI-4889d4?style=for-the-badge&logo=udacity&logoColor=white)](https://www.udacity.com/masters-artificial-intelligence)

![Total Required](https://img.shields.io/badge/Total_Hours_Required-1%2C120-blueviolet?style=for-the-badge&logo=clockify&logoColor=white) ![My Total](https://img.shields.io/badge/My_Hours_Total-1%2C187-blueviolet?style=for-the-badge&logo=clockify&logoColor=white)

![Core Required](https://img.shields.io/badge/Core-370_Hours-9cf?style=flat-square) ![Electives Required](https://img.shields.io/badge/Electives-750%2B_Hours-9cf?style=flat-square) ![My Electives](https://img.shields.io/badge/My_Electives-817_Hours-85d5ad?style=flat-square)

_✨ Exceeding elective requirements by 67 hours_

---

## 📊 My Program Progress

![Total Hours](https://img.shields.io/badge/Total_Hours-198%2F1%2C187-blueviolet?style=for-the-badge&logo=clockify&logoColor=white) ![Overall Progress](https://img.shields.io/badge/Overall_Progress-16.7%25-red?style=for-the-badge)

![Core: In Progress](https://img.shields.io/badge/Core-3%2F7_Complete_%7C_159%2F370_Hours-9cf?style=flat-square) ![Electives: In Progress](https://img.shields.io/badge/Electives-1%2F14_Complete_%7C_39%2F817_Hours-9cf?style=flat-square)

---

From Udacity's website:

> Build your foundation with 7 industry-relevant Nanodegrees covering AI programming, data science, machine learning, cloud, and applied AI use cases.

---

## 🎯 Core Nanodegree Programs (7 Required)

### ✅ [AI Programming with Python](https://www.udacity.com/course/ai-programming-python-nanodegree--nd089)

![Started](https://img.shields.io/badge/Started-Oct_2025-9cf?style=flat-square) ![Completed](https://img.shields.io/badge/Completed-Dec_2025-85d5ad?style=flat-square) ![Hours](https://img.shields.io/badge/Hours-52-blueviolet?style=flat-square&logo=clockify) [![Certificate](https://img.shields.io/badge/📜_View_Certificate-85d5ad?style=flat-square&logo=acclaim)](https://www.udacity.com/certificate/e/eda262f4-b5de-11f0-96a9-7336b744cdbb)

<details>
<summary>📚 Key Skills</summary>

- **Programming**: Python, NumPy, Pandas, Matplotlib
- **Mathematics**: Linear Algebra, Calculus (original course before Dec. 2025 update)
- **Neural Networks**: PyTorch, Model Training
- **Development**: Jupyter Notebooks, Git
</details>
<p> </p>
<p><b>Note:</b> This course was updated December 12, 2025 when I was 96% done. I decided to restart it to get the new project content.</p>

---

### ✅ [Statistics for Data Analysis](https://www.udacity.com/course/statistics-for-data-analysis-nanodegree--nd588)

![Started](https://img.shields.io/badge/Started-Nov_2025-9cf?style=flat-square) ![Completed](https://img.shields.io/badge/Completed-Nov_2025-85d5ad?style=flat-square) ![Hours](https://img.shields.io/badge/Hours-64-blueviolet?style=flat-square&logo=clockify) [![Certificate](https://img.shields.io/badge/📜_View_Certificate-85d5ad?style=flat-square&logo=acclaim)](https://www.udacity.com/certificate/e/0a89c998-c863-11f0-aeb6-673c162c3cea)

<details>
<summary>📚 Key Skills</summary>

- **Statistics**: Descriptive & Inferential Statistics, Probability
- **Data Analysis**: Hypothesis Testing, A/B Testing, Regression Analysis
- **Visualization**: Data Storytelling, Statistical Charts
- **Tools**: Python, SQL, Excel
</details>

---

### ⬜ [Introduction to Machine Learning with PyTorch](https://www.udacity.com/course/intro-to-machine-learning-nanodegree--nd229)

![Hours](https://img.shields.io/badge/Hours-49-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **ML Fundamentals**: Supervised & Unsupervised Learning
- **Algorithms**: Classification, Regression, Clustering
- **Deep Learning**: Neural Networks, PyTorch
- **Model Development**: Feature Engineering, Model Evaluation, Hyperparameter Tuning
</details>

---

### ✅ [Data Analyst](https://www.udacity.com/course/data-analyst-nanodegree--nd002)

![Started](https://img.shields.io/badge/Started-Nov_2025-9cf?style=flat-square) ![Completed](https://img.shields.io/badge/Completed-Dec_2025-85d5ad?style=flat-square) ![Hours](https://img.shields.io/badge/Hours-43-blueviolet?style=flat-square&logo=clockify) [![Certificate](https://img.shields.io/badge/📜_View_Certificate-85d5ad?style=flat-square&logo=acclaim)](https://www.udacity.com/certificate/e/54b00826-cbf5-11f0-8cf2-93d6a3ddb582)

<details>
<summary>📚 Key Skills</summary>

- **Data Wrangling**: Data Cleaning, ETL Processes
- **Analysis**: Exploratory Data Analysis, Statistical Analysis
- **Visualization**: Tableau, Python (Matplotlib, Seaborn)
- **Database**: SQL, Query Optimization
</details>

<p></p>

**🔗 Projects**:

- [Project 1: Investigate a Dataset - TMDb Movie Data](https://github.com/kellewic/data-analyst-nanodegree-projects/tree/main/project1)
- [Project 2: Real-World Data Wrangling - Movie Data](https://github.com/kellewic/data-analyst-nanodegree-projects/tree/main/project2)
- [Project 3: The Streaming Era Divide - Data Visualization](https://github.com/kellewic/data-analyst-nanodegree-projects/tree/main/project3)

---

### ⬜ [Deep Learning](https://www.udacity.com/course/deep-learning-nanodegree--nd101)

![Hours](https://img.shields.io/badge/Hours-61-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Neural Networks**: CNNs, RNNs, LSTMs, GANs
- **Frameworks**: PyTorch, TensorFlow
- **Applications**: Computer Vision, NLP, Time Series
- **Optimization**: Training Strategies, Transfer Learning
</details>

---

### ⬜ [Generative AI](https://www.udacity.com/course/generative-ai--nd608)

![Hours](https://img.shields.io/badge/Hours-50-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Models**: GPT, DALL-E, Stable Diffusion, LLMs
- **Techniques**: Prompt Engineering, Fine-tuning, RAG
- **Frameworks**: LangChain, Hugging Face
- **Applications**: Text Generation, Image Generation, AI Assistants
</details>

---

### ⬜ [Agentic AI](https://www.udacity.com/course/agentic-ai--nd900)

![Hours](https://img.shields.io/badge/Hours-51-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Agent Design**: Multi-Agent Systems, Autonomous Agents
- **Frameworks**: LangGraph, AutoGen
- **Integration**: API Integration, Tool Usage
- **Applications**: AI Workflows, Agent Orchestration
</details>

---

From Udacity's website:

> Customize your journey by choosing from a wide set of electives. Complete at least 750 hours across AI, data science, engineering, and business programs to deepen your expertise where it matters most.

---

## 🚀 Elective Nanodegree Programs (14 Chosen, 817 Hours)

### ✅ [React](https://www.udacity.com/course/react-nanodegree--nd019)

![Started](https://img.shields.io/badge/Started-Nov_2025-9cf?style=flat-square) ![Completed](https://img.shields.io/badge/Completed-Nov_2025-85d5ad?style=flat-square) ![Hours](https://img.shields.io/badge/Hours-39-blueviolet?style=flat-square&logo=clockify) [![Certificate](https://img.shields.io/badge/📜_View_Certificate-85d5ad?style=flat-square&logo=acclaim)](https://www.udacity.com/certificate/e/0991438e-b979-11f0-ac4c-0bfb99d86a73)

<details>
<summary>📚 Key Skills</summary>

- **Frontend Development**: React, JSX, Component Architecture
- **State Management**: Redux, Context API, Hooks
- **Routing & Navigation**: React Router
- **Testing**: Jest, React Testing Library, TDD
- **Mobile Development**: React Native (Extracurricular)
</details>

<p></p>

**🔗 Projects**:

- [Project 1: MyReads - Book Tracking App](https://github.com/kellewic/react-nanodegree-project1)
- [Project 2: Employee Polls Web App](https://github.com/kellewic/react-nanodegree-project2)

---

### ⬜ [Introduction to Machine Learning with TensorFlow](https://www.udacity.com/course/intro-to-machine-learning-with-tensorflow-nanodegree--nd230)

![Hours](https://img.shields.io/badge/Hours-49-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **ML Fundamentals**: Supervised & Unsupervised Learning
- **Framework**: TensorFlow, Keras
- **Model Development**: Feature Engineering, Model Evaluation
- **Deployment**: TensorFlow Lite, Model Serving
</details>

---

### ⬜ [Artificial Intelligence](https://www.udacity.com/course/ai-artificial-intelligence-nanodegree--nd898)

![Hours](https://img.shields.io/badge/Hours-40-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Search Algorithms**: A\*, Minimax, Alpha-Beta Pruning
- **Planning**: Classical Planning, Constraint Satisfaction
- **Reasoning**: Logic, Probabilistic Reasoning, Bayesian Networks
- **Applications**: Game Playing AI, Planning Systems
</details>

---

### ⬜ [Natural Language Processing](https://www.udacity.com/course/natural-language-processing-nanodegree--nd892)

![Hours](https://img.shields.io/badge/Hours-53-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **NLP Fundamentals**: Tokenization, Embeddings, Transformers
- **Models**: BERT, GPT, seq2seq
- **Applications**: Sentiment Analysis, Translation, Question Answering
- **Tools**: Hugging Face, spaCy, NLTK
</details>

---

### ⬜ [Agentic AI Engineer with LangChain and LangGraph](https://www.udacity.com/course/agentic-ai-engineer-with-langchain-and-langgraph--nd901)

![Hours](https://img.shields.io/badge/Hours-26-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Frameworks**: LangChain, LangGraph
- **Agent Development**: Tool Creation, Agent Orchestration
- **LLM Integration**: Prompt Engineering, Chain Building
- **Applications**: AI Assistants, Workflow Automation
</details>

---

### ⬜ [Data Architect](https://www.udacity.com/course/data-architect-nanodegree--nd038)

![Hours](https://img.shields.io/badge/Hours-73-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Database Design**: Data Modeling, Schema Design, Normalization
- **Data Warehousing**: Dimensional Modeling, ETL, Star Schema
- **Cloud Platforms**: AWS, Azure, Data Lakes
- **Governance**: Data Quality, Security, Compliance
</details>

---

### ⬜ [Data Engineering with AWS](https://www.udacity.com/course/data-engineer-nanodegree--nd027)

![Hours](https://img.shields.io/badge/Hours-39-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **AWS Services**: S3, Redshift, EMR, Glue, Lambda
- **Data Pipelines**: Airflow, ETL/ELT Processes
- **Big Data**: Spark, Distributed Computing
- **Storage**: Data Lakes, Data Warehouses
</details>

---

### ⬜ [Data Streaming](https://www.udacity.com/course/data-streaming-nanodegree--nd029)

![Hours](https://img.shields.io/badge/Hours-53-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Stream Processing**: Apache Kafka, Flink, Spark Streaming
- **Real-time Analytics**: Event Processing, Stream Joins
- **Architecture**: Event-Driven Design, Microservices
- **Deployment**: Kubernetes, Cloud Streaming Services
</details>

---

### ⬜ [AWS Machine Learning Engineer](https://www.udacity.com/course/aws-machine-learning-engineer-nanodegree--nd189)

![Hours](https://img.shields.io/badge/Hours-94-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **AWS ML Services**: SageMaker, Rekognition, Comprehend
- **ML Pipeline**: Training, Tuning, Deployment, Monitoring
- **Infrastructure**: Lambda, ECR, Model Endpoints
- **MLOps**: CI/CD for ML, Model Versioning
</details>

---

### ⬜ [Machine Learning DevOps Engineer](https://www.udacity.com/course/machine-learning-dev-ops-engineer-nanodegree--nd0821)

![Hours](https://img.shields.io/badge/Hours-63-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **MLOps**: CI/CD Pipelines, Model Deployment, Monitoring
- **Infrastructure**: Docker, Kubernetes, Cloud Platforms
- **Automation**: Testing, Model Validation, Drift Detection
- **Best Practices**: Version Control, Reproducibility, Scalability
</details>

---

### ⬜ [Machine Learning Model Optimization](https://www.udacity.com/course/machine-learning-model-optimization--nd446)

![Hours](https://img.shields.io/badge/Hours-48-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Optimization**: Model Pruning, Quantization, Knowledge Distillation
- **Performance**: Latency Reduction, Memory Efficiency
- **Deployment**: Edge Devices, Mobile ML, TensorFlow Lite
- **Tools**: ONNX, TensorRT, Model Compression
</details>

---

### ⬜ [Data Scientist](https://www.udacity.com/course/data-scientist-nanodegree--nd025)

![Hours](https://img.shields.io/badge/Hours-61-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Data Science**: Exploratory Analysis, Feature Engineering, Model Selection
- **ML Algorithms**: Ensemble Methods, Deep Learning, NLP
- **Tools**: Python (Scikit-learn, Pandas, NumPy), SQL, Spark
- **Communication**: Data Visualization, Storytelling, Business Insights
</details>

---

### ⬜ [Deep Reinforcement Learning](https://www.udacity.com/course/deep-reinforcement-learning-nanodegree--nd893)

![Hours](https://img.shields.io/badge/Hours-83-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **RL Fundamentals**: MDP, Q-Learning, Policy Gradients
- **Algorithms**: DQN, A3C, PPO, DDPG, SAC
- **Applications**: Game AI, Robotics, Control Systems
- **Frameworks**: OpenAI Gym, Unity ML-Agents
</details>

---

### ⬜ [AI Trading Strategies](https://www.udacity.com/course/ai-trading-strategies--nd881)

![Hours](https://img.shields.io/badge/Hours-96-lightgrey?style=flat-square&logo=clockify)

<details>
<summary>📚 Key Skills</summary>

- **Financial Analysis**: Time Series, Risk Management, Portfolio Optimization
- **ML for Finance**: Predictive Modeling, Sentiment Analysis, Alpha Factors
- **Algorithms**: Algorithmic Trading, Backtesting, Market Making
- **Tools**: Python (Pandas, NumPy), APIs, Trading Platforms
</details>

---

## 🛠️ Technical Skills Acquired

### Languages & Frameworks

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat&logo=react&logoColor=black)
![Redux](https://img.shields.io/badge/-Redux-764ABC?style=flat&logo=redux&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)

### Tools & Platforms

![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/-VS%20Code-007ACC?style=flat&logo=visual-studio-code&logoColor=white)
![Jupyter](https://img.shields.io/badge/-Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)

### Testing

![Jest](https://img.shields.io/badge/-Jest-C21325?style=flat&logo=jest&logoColor=white)
![Testing Library](https://img.shields.io/badge/-Testing%20Library-E33332?style=flat&logo=testing-library&logoColor=white)

---

## 📈 Learning Journey

```
2025
└── Oct: 🔄 Started AI Programming with Python
└── Nov: ✅ Completed React Nanodegree
└── Nov: ✅ Completed Statistics for Data Analysis
└── Dec: ✅ Completed Data Analyst
└── Dec: ✅ Completed AI Programming with Python
```

---

<div align="center">

**Last Updated**: December 2025

_Committed to continuous learning_

</div>
